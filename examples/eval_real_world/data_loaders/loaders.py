...
from sklearn.datasets import fetch_california_housing, fetch_openml, load_diabetes as _load_diabetes
...
# ---------------------------------------------------------------------------
def _load_hf_regression(
    config: str, target_col: str
...
def load_synthetic_spatial(
    ...
    signal = (
        np.sin(2.0 * np.pi * X[:, 0]) * np.cos(2.0 * np.pi * X[:, 1])
        + 0.5
        * np.sin(20.0 * np.pi * X[:, 0])
